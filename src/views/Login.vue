...
<template>
  <div class="login" id="login">
    ...
    <div class="log-bg">
      ...
      <div class="log-text">项目进度查看系统</div>
    </div>
    <div class="log-email">
      <input
        type="text"
...
export default {
  ...
  methods: {
    ...
    //登录请求
    toLogin() {
        if(this.account == dataJson.login.name && this.password == dataJson.login.password){
            this.$router.push({name:'index' })
            const dateYear = formatTime(new Date(), 'yyyy-MM-dd')
            const data = encode(dateYear)
            localStorage.setItem(dataJson.login.name,data)
        }else{
            alert('密码或帐号错误')
        }
    },
